...
import DeleteModal from '../components/Pembeli-DeleteDeals';

function PembeliMain() {
  const { id } = useParams();
  const dispatch = useDispatch();
  const [deal, setDeal] = useState({});
  const [dealsId, setDealsId] = useState(0);
  ...
  return ( 
    <>
      <LoggedInNavbar />
      {deleteModal === true ? <DeleteModal openDeleteRequest={handleDeleteBid} id={dealsId} /> : null}
          ...
          <main className = "pt-10 max-w-7xl mx-auto px-4 lg:px-8">
            <div className = "flex items-baseline justify-between pt-24 pb-6 border-b border-gray-200">
              <h1 className = "text-3xl font-extrabold tracking-tight text-teal-700">
                MyDeals <span className = "font-normal"> Checkout </span>
              </h1>

              <div className = "flex items-center">
                <button type = "button" className = "p-2 -m-2 ml-4 sm:ml-6 text-gray-400 hover:text-gray-500 lg:hidden">
                  <span className = "sr-only" > Filters </span> {/* <!-- Heroicon name: solid/filter --> */ } 
                  <svg className = "w-5 h-5" aria-hidden = "true" xmlns = "http://www.w3.org/2000/svg" viewBox = "0 0 20 20" fill = "currentColor">
                    <path fillRule = "evenodd" d = "M3 3a1 1 0 011-1h12a1 1 0 011 1v3a1 1 0 01-.293.707L12 11.414V15a1 1 0 01-.293.707l-2 2A1 1  018 17v-5.586L3.293 6.707A1 1 0 013 6V3z" clipRule = "evenodd"/>
                  </svg> 
                </button> 
              </div> 
            </div>
                    ...
                    <div className = "sm:-mx-6 lg:-mx-8">
                      <div className = "align-middle  sm:px-6 lg:px-8" >
                        <div className = "flex flex-row align-top mb-5 h-6 w-60 max-w-max text-teal-600 hover:text-teal-400 cursor-pointer transition duration-150 ease-in-out" >
                          ...
                          <Link to = "/deals/pembeli" className = "mb-3 font-semibold text-lg" > Daftar MyDeals saya </Link> 
                        </div>
                                    ...
                                    <div>
                                      ...
                                      <section aria-labelledby = "information-heading" className = "mt-1">
                                        <p className = "mt-1 text-xs text-gray-500 font-normal" > 
                                          milik <span className = "font-bold text-teal-600 hover:text-teal-500 hover:underline cursor-pointer">{ deal.seller_email }</span>
                                        </p >
                                        <div className = "mt-6" >
                                          <p className = "text-sm font-bold text-teal-600" > Product Description </p> 
                                          <p className = "text-sm font-normal text-gray-500" > { deal.description } </p> 
                                        </div>
                                        ...
                                        <div className = "border-t border-solid border-gray-200 mt-6 pt-2" >
                                          ...
                                          <div className = "mt-3" >
                                            <p className = "text-sm font-bold text-teal-600" >
                                              Jumlah Deal: <span className = "text-sm font-normal text-gray-500" > { deal.deal_qty } </span>
                                            </p >
                                          </div>
                                          <div className = "mt-3" >
                                            <p className = "text-sm font-bold text-teal-600" >
                                              Harga Deal: <span className = "text-sm font-normal text-gray-500" >Rp. {(deal.deal_price) ? deal.deal_price.toLocaleString(): '' } </span>
                                            </p> 
                                          </div> 
                                          {/* <div className = "mt-3" >
                                            <p className = "text-sm font-bold text-teal-600" >
                                              Harga Ongkir: < span className = "text-sm font-normal text-gray-500" > Rp39000 </span>
                                            </p>
                                          </div>  */}
                                          <div className = "mt-3" >
                                            <p className = "text-sm font-bold text-teal-600" > 
                                              Waktu Deal: <span className = "text-sm font-normal text-gray-500" > { dateFormatter(deal.createdAt) } </span>
                                            </p >
                                          </div> 
                                          <div className = "mt-3" >
                                            <p className = "text-sm font-bold text-teal-600" >
                                              ...
                                              <span className = "text-sm pl-2 text-gray-500" >
                                                <span className = "px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-gray-600" > { deal.payment_status}</span> 
                                              </span> 
                                            </p> 
                                          </div> 
                                        </div>
                                ...
                                <div>
                                  <section aria-labelledby = "options-heading" >
                                    <div className = "flex flex-row justify-between align-top" >
                                      <p className = "text-left mb-1 text-md font-semibold" > 
                                        Total harga Deal <span className = "text-xs font-light" > (termasuk ongkos kirim) </span>
                                      </p>
                                    </div>
                                    ...
                                    <p className = " text-center mt-1 text-xs text-gray-500 font-normal" > 
                                      {(deal.payment_status !== 'paid') ? "Kamu akan dialihkan ke 3rd party payment gateway untuk pembayaran." : "" } 
                                    </p>
                                  </section> 
                                </div>
